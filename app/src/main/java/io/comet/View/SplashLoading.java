...
public class SplashLoading extends FrameLayout {
    ...
    public SplashLoading(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }

    public SplashLoading(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView(context);
    }

    private void initView(Context context) {
        LayoutInflater.from(context).inflate(R.layout.splash_loading, this, true);
    ...
    static class BookHandler extends Handler {
        private WeakReference<SplashLoading> weakReference;

        public BookHandler(SplashLoading splashLoading) {
            weakReference = new WeakReference<>(splashLoading);
        }

        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            SplashLoading splashLoading = weakReference.get();
            if (null == splashLoading)
                return;
            splashLoading.playAnim();
